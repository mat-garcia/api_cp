...
public abstract class AbstractGenericCMJPADAO<T> implements GenericDAO<T>, Serializable {
	...
	protected static Logger logger = Logger.getLogger(AbstractGenericCMJPADAO.class);
	...
	public AbstractGenericCMJPADAO(DataSourceEnum dataSource) {
	...
	public void startTransaction() {
		...
		if (dataSource.equals(DataSourceEnum.ORACLE)) {
			if (!HibernateUtilCMOracle.getInstance().getSessionFactory().getCurrentSession().getTransaction()
					.isActive()) {
				getSession().beginTransaction();
			}
		} else if (dataSource.equals(DataSourceEnum.SQL_SERVER) ) {
			if (!HibernateUtilCMSqlServer.getInstance().getSessionFactory().getCurrentSession().getTransaction()
					.isActive()) {
				getSession().beginTransaction();
			}
		}
	...
	public void finishTransacton() {
		...
		if (dataSource.equals(DataSourceEnum.ORACLE)) {
			if (HibernateUtilCMOracle.getInstance().getSessionFactory().getCurrentSession().getTransaction()
					.isActive()) {
				HibernateUtilCMOracle.getInstance().getSessionFactory().getCurrentSession().getTransaction().commit();
			}
		} else if (dataSource.equals(DataSourceEnum.SQL_SERVER) ) {
			if (HibernateUtilCMSqlServer.getInstance().getSessionFactory().getCurrentSession().getTransaction()
					.isActive()) {
				HibernateUtilCMSqlServer.getInstance().getSessionFactory().getCurrentSession().getTransaction()
						.commit();
			}
		}
	...
	public Session getSession() {
		...
		if (dataSource.equals(DataSourceEnum.ORACLE)) {
				session = HibernateUtilCMOracle.getInstance().getSessionFactory().getCurrentSession();
			
		} else if (dataSource.equals(DataSourceEnum.SQL_SERVER) ) {
				session = HibernateUtilCMSqlServer.getInstance().getSessionFactory().getCurrentSession();
	...
	public void delete(T clazz, Integer id) throws PersistenceException {
		try {
			...
			/*
			 * getSession(). PublicSupport.
			 * executeUpdate("delete PublicSupport c where c.id = :publicSupportId"
			 * , [publicSupportId:publicSupport.id])
			 */
